...
def init_weights(shape):
    return tf.Variable(initial_value = tf.random_normal(shape, stddev=0.001))

class Layers:
    def __init__(self, w1, w2, w3, w4, w5, w6, wo, p_keep_conv, p_keep_hidden):
        self.w1 = w1
        self.w2 = w2
        self.w3 = w3
        self.w4 = w4
    ...
    def esimate(self, X):
        ...
        l1 = tf.nn.avg_pool(l1a, ksize=[1, 2, 2, 1],  # l2 shape=(?, 7, 7, 64)
                            strides=[1, 2, 2, 1], padding='SAME')

        l2a = tf.nn.conv2d(l1, self.w2,  # l2a shape=(?, 14, 14, 64)
                                      strides=[1, 1, 1, 1], padding='SAME')

        l3a = tf.nn.relu(tf.nn.conv2d(l2a, self.w3,  # l2a shape=(?, 14, 14, 64)
                                      strides=[1, 1, 1, 1], padding='SAME'))
        ...
        l4a = tf.nn.relu(tf.nn.conv2d(l3, self.w4,  # l2a shape=(?, 14, 14, 64)
                                      strides=[1, 1, 1, 1], padding='SAME'))
        l4 = tf.nn.max_pool(l4a, ksize=[1, 3, 3, 1],  # l2 shape=(?, 7, 7, 64)
                            strides=[1, 3, 3, 1], padding='SAME')
        l4 = tf.nn.dropout(l4, self.p_keep_conv)
...
w1 = init_weights([8, 8, 3, 32])       # 3x3x1 conv, 32 outputs
w2 = init_weights([8, 8, 32, 64])     # 3x3x32 conv, 64 outputs
w3 = init_weights([8, 8, 64, 128])    # 3x3x32 conv, 128 outputs
w4 = init_weights([8, 8, 128, 128])    # 3x3x32 conv, 128 outputs
w5 = init_weights([8, 8, 128, 128])    # 3x3x32 conv, 128 outputs
w6 = init_weights([128 * 12 * 12, 1024]) # FC 128 * 4 * 4 inputs, 625 outputs
wo = init_weights([1024, 2])         # FC 625 inputs, 10 outputs (labels)

p_keep_conv = tf.constant(.7)
p_keep_hidden = tf.constant(.5)
layers = Layers(w1, w2, w3, w4, w5, w6, wo, p_keep_conv, p_keep_hidden)
...
py_x = tf.clip_by_value(tf.nn.softmax(py_x), 1e-10, 1.0 )
cost = -tf.reduce_sum(Y*tf.log(py_x))
#train_op = tf.train.AdamOptimizer(0.005, 0.90).minimize(cost)
#train_op = tf.train.RMSPropOptimizer(0.005).minimize(cost)
